...
export default function RuntimeEnhancements(){
 ...
 useEffect(()=>{
  let last="";
  const sync=()=>{normalizeNavigation();const next=detectView();normalizeDataCopy(next);if(next!==last){last=next;setView(next)};const content=document.querySelector("section.content");content?.classList.toggle("runtime-assets-active",next==="assets");setHost((next==="assets"||next==="backup")?ensureHost(next):null)};
